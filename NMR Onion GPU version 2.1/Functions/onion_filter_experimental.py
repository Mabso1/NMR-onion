...
def filter_noise(y,noise_region,spectrum,fs,SF,O1p):
   ...
   # for i in range(0,1000):
    ...
    noise_out=noise_als
     ...
     # --- Improved baseline noise filter ---
    # Extract super-Gaussian filter from spectrum
    sg = spectrum[0]
    
    # Ensure sg is the same shape as noise
    if sg.shape != noise_als.shape:
        sg = ng.proc_base.zf_size(sg, len(noise_als))
 
    # Use a *smoothed* complementary mask to avoid edge artifacts
    noise_mask = 1 - sg
    noise_mask_smooth = gaussian_filter1d(noise_mask, sigma=5)
 
    # Apply smooth complementary mask to synthetic noise
    noise_als *= noise_mask_smooth
...
def onion_filter(low_ppm,high_ppm,noise_region,y,fs,SF,O1p,offset=False,baseline_raw=False):
    
    if (baseline_raw==True):
        y_fft_raw=np.fft.fftshift(np.fft.fft(y))
        baseline_estimate=np.real(np.median(y_fft_raw))
        y_fft_raw=y_fft_raw-baseline_estimate
        y=np.fft.ifft(np.fft.ifftshift(y_fft_raw))
    ...
    if (offset==True):
        y_fft_filt=np.fft.fftshift(np.fft.fft((FID_filtered_output)))
        
        baseline_estimate=np.real(np.median(y_fft_filt))
        
        y_fft_filt=y_fft_filt-baseline_estimate
        
    
    return FID_filtered_output,syn_noise_level

def onion_filter_expaned(low_ppm,high_ppm,noise_region,y,fs,SF,O1p,offset=False,baseline_raw=False, minimum_filter=False):
    
    FID_filtered_output,syn_noise_level=onion_filter(low_ppm,high_ppm,noise_region,y,fs,SF,O1p,offset,baseline_raw)
    
    if (minimum_filter==True):
       y_fft_filt=np.fft.fftshift(np.fft.fft((FID_filtered_output))) 
       baseline_estimate = minimum_filter1d(np.real(y_fft_filt), size=51)
       y_fft_filt+=y_fft_filt-2.0*baseline_estimate 
       FID_filtered_output=np.fft.ifft(np.fft.ifftshift(y_fft_filt))
    
    if (minimum_filter==False):
        baseline_estimate=0
        
    if (offset==True):
        y_fft_filt=np.fft.fftshift(np.fft.fft((FID_filtered_output))) 
        baseline_estimate=np.median(y_fft_filt)
        y_fft_filt=y_fft_filt-baseline_estimate
        FID_filtered_output=np.fft.ifft(np.fft.ifftshift(y_fft_filt))
        
        
    return  FID_filtered_output*0.5,syn_noise_level,baseline_estimate

def onion_filter_expaned_offset(low_ppm,high_ppm,noise_region,data,fs,SF,O1p,offset=True,minimum_filter=False):
    ...
    if (offset==True):
        ...
        y_filt1,noise_level,baseline_estimate=onion_filter_expaned(low_ppm, high_ppm, noise_region, data, fs, SF, O1p,offset=False,baseline_raw=False,minimum_filter=False)
    
        y_fft_filt=np.fft.fftshift(np.fft.fft((y_filt)))
        y_fft_filt1=np.fft.fftshift(np.fft.fft((y_filt1)))
        
        odiff=np.fft.ifft(np.fft.ifftshift(y_fft_filt1))-np.fft.ifft(np.fft.ifftshift(y_fft_filt))
        
        y_filt=y_filt+odiff
    
    if (offset==False):
        y_filt,noise_level,baseline_estimate=onion_filter_expaned(low_ppm, high_ppm, noise_region, data, fs, SF, O1p,offset=False,baseline_raw=False,minimum_filter=False)
        
    if (minimum_filter==True):
        y_filt,noise_level,baseline_estimate=onion_filter_expaned(low_ppm, high_ppm, noise_region, data, fs, SF, O1p,offset=False,baseline_raw=False,minimum_filter=True)
        y_filt1,noise_level,baseline_estimate=onion_filter_expaned(low_ppm, high_ppm, noise_region, data, fs, SF, O1p,offset=False,baseline_raw=False,minimum_filter=True)
