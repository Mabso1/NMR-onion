...
def plot_data(ylim1,ylim2,xlim1,xlim2,ppm_val,y_fft_raw,t,data,FID=True):
    plt.xlabel('ppm')  
    plt.ylabel('Intensity')
    plt.title("NMR Spectrum")  
    plt.plot(np.flip(ppm_val),np.flip(y_fft_raw),color="blue")
    plt.xlim(xlim2,xlim1) # zoom x-axis
    plt.ylim(ylim1,ylim2) # zoom y-axis
    plt.show()
...
def plot_filter(ylim1,ylim2,low_ppm,high_ppm,ppm_val,y_fft_raw,y_fft_filt):
    plt.xlabel('ppm')  
    plt.ylabel('Intensity')
    plt.title("Filtered NMR Spectrum")  
    plt.plot(np.flip(ppm_val),np.flip(y_fft_raw),color="blue")
    plt.plot(np.flip(ppm_val),np.flip(y_fft_filt),color="green")


    plt.xlim(high_ppm,low_ppm) # zoom x-axis
    plt.ylim(ylim1,ylim2) # zoom y-axis
    plt.show()
