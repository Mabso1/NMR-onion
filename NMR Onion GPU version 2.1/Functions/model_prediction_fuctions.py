...
def spectrum_prediction(model_name,par_res,y_filt,t,k,optional_par=False):
    
    # get parameter output as vector  
    par_hat=par_hat_output(best_model=model_name,par_res=par_res)

    # get FID prediction
    y_hat=pred(theta=par_hat,m=k,t=t,y=y_filt/np.linalg.norm(y_filt), model_name=model_name)

    # get individual sinusoids 
    single_sinusoids=single_sinusoid(theta=par_hat,m=k,t=t,y=y_filt/np.linalg.norm(y_filt), model_name=model_name)
    A,Z=preds_split(theta=par_hat,m=k,t=t,y=y_filt/np.linalg.norm(y_filt), model_name=model_name)
    
    if (optional_par==True):
    ...
    return par_hat,y_hat,single_sinusoids,amps,phases
