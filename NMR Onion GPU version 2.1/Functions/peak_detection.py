...
def integrate_peaks(xlim1_ppm, xlim2_ppm, ylim1, ylim2, single_sinusoids,y_filt,
                    ppm_val, par_res, SF, O1p, rel_height=0.001, plot=True,
    ...
    for i in range(k):
        y0_mag = np.real(np.fft.fftshift(np.fft.fft(single_sinusoids[i]*np.linalg.norm(y_filt))))
        omega_ppm=ppm_val[np.argmax(y0_mag)]
        #areas, bounds = auto_integrate_peaks(ppm_val, y0_mag, known_peaks=[omega_ppm[i]], rel_height=rel_height)
        areas, bounds = auto_integrate_peaks(ppm_val, y0_mag, known_peaks=[omega_ppm], rel_height=rel_height)
        #print(omega_ppm)
        ...
        if plot:
            plt.plot(ppm_val, y0_mag, label='Real(FFT)' if i == 0 else None)
        
            # Create a mask to mark integrated regions
            mask = np.zeros_like(ppm_val, dtype=bool)
            for left, right in bounds:
                mask |= (ppm_val >= left) & (ppm_val <= right)
        
            # Fill under the curve only in integrated areas
            plt.fill_between(ppm_val, y0_mag, where=mask, color='orange', alpha=0.5,
                             label='Integrated Area' if i == 0 else None)
...
def onion_peak_detection(width, noise_peaks, cut, ylim1, ylim2, y_fft_filt, y_filt,
    ...
    if plot:
        ...
        plt.plot(np.flip(ppm_val), np.flip(y_fft_filt), color="blue")
