...
path="/home/mabso/Desktop/NMR-onion-main/NMR Onion GPU version 2.1/Functions" # set to the path in which NMR onion folder is stored
sys.path.append(path)
#import nmrglue as ng
from plotting_functions import plot_results, plot_model_bootstrap,plot_data,plot_filter
...
from onion_filter_experimental import onion_filter_expaned_offset

#%%
# set your own data_path
data_path="/home/mabso/Desktop/NMR-onion-main/NMR-onion/data/dataset 2/DTU800_150922_CHG_MMS052002/10/pdata/1"
...
y_fft_raw=y_fft_raw-np.median(y_fft_raw) # baseline correct if needed 

# check the data import works by plotting
plot_data(ylim1=0, ylim2=0.5*10**8, xlim1=0, xlim2=12, ppm_val=ppm_val, y_fft_raw=y_fft_raw, t=t, data=data)
...
y_filt,noise_level =onion_filter_expaned_offset(low_ppm,high_ppm,noise_region,data,fs,SF,O1p,offset=True,minimum_filter=False)

y_fft_filt=np.fft.fftshift(np.fft.fft((y_filt)))

plot_filter(ylim1=0,ylim2=0.05*10**8,high_ppm=high_ppm,low_ppm=low_ppm,ppm_val=ppm_val,y_fft_raw=y_fft_raw,y_fft_filt=y_fft_filt)
...
omega_hz_filtered=onion_peak_detection(width=1.0, noise_peaks=15,cut=0,ylim1=ylim1, ylim2=ylim2, y_fft_filt=y_fft_filt,
                                       low_ppm=low_ppm, high_ppm=high_ppm, ppm_val=ppm_val,
                                       t=t, fs=fs,freq=freq,y_filt=y_filt,noise_level=noise_level,SF=SF,O1p=O1p,plot=True)
...
plot_results(xlim1_ppm,xlim2_ppm, ylim1,ylim2,xlim1,xlim2,y_hat=y_hat,y_filt=y_filt,
    ...
    freq_domain_ppm=True,show_peaks=True, residuals=True,save_dir=None,save_format="pdf")
...
omega_ppm, omega_hz, int_list= integrate_peaks(
    ...
    ylim1=ylim1, ylim2=1800000,
    single_sinusoids=single_sinusoids,y_filt=y_filt,
    ppm_val=ppm_val,
    par_res=par_res,
    SF=SF,
    O1p=O1p,
