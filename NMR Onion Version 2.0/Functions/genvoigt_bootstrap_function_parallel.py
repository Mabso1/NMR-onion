...
def multi_wild_fit(par_hat, fs, t, k, y,B,cores):
    if __name__ == '__main__':
        with tqdm_joblib(tqdm(desc="My calculation", total=B)) as progress_bar:
            fit=Parallel(n_jobs=cores,verbose=100)(delayed(fit_parameter_error(par_hat=par_hat, fs=fs_new, t=t_new, k=k, y=y_dem_list[l])) for l in range(0,B))
    return (fit)




def genvoigt_wild_boostrap(B,CI_level,alpha,omega,eta,scale1,scale2,t,k,y,fs,model_name,low_ppm,high_ppm,freq,SF,O1p,parallel,cores=1):
    Boots=[]
    ...
    if(parallel==False):
        ...
        for l in range(0,B):
         ...
         for l in range(0,B):
             ...
             y_dem,fs_new,tn_new,t_new,freq_new,hz_moved=signal_decimate(low_ppm=low_ppm,high_ppm=high_ppm, y_filt=y_resamp, freq=freq, SF=SF, O1p=O1p, fs=fs)
             y_dem_list.append(y_dem)
             
             
         wild_fit=multi_wild_fit(par_hat=par_hat, fs=fs_new, t=t_new, k=k, y=y_dem_list,B=B,cores=cores)
         
         for l in range(0,B):
            par_res=par_est(fit=wild_fit[l], k=k, model_name=model_name)
            omega_boot=par_res['omega_est']-hz_moved
            alpha_boot=expit_sci(par_res['alpha_est']) #convert with expit_sci
            eta_boot=Softplus(par_res['eta_est'])  # convert to Softplus
            scale1_boot=par_res['scale'] # uncomment if model is skewed 
...
def fit_parameter_error(par_hat,fs,t,k,y):
    ...
    train_load=DataLoader(train_data,batch_size=N,shuffle=False)
