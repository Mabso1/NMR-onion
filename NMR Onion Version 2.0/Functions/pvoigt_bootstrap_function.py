...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct 12 10:25:12 2022
...
def pvoigt_wild_boostrap(B,CI_level,alpha,omega,eta,scale1,scale2,t,k,y,fs,model_name,low_ppm,high_ppm,freq,SF,O1p,parallel,cores=1):
    Boots=[]
    y_resamps=[]
   ...
   # par_hat=np.concatenate(np.array([alpha,omega+hz_moved,eta,scale1,scale2])) # collect the parameters
    ...
    for l in range(0,B):
        ...
        alpha_boot=expit_sci(par_res['alpha_est']) # convert to expit_sci
        eta_boot=expit_sci(par_res['eta_est'])  # convert to expit_sci
        scale1_boot=par_res['scale'] 
        scale2_boot=par_res['scale1'] 
 ...
 #   boot_std=boot_res.std() # get parameter std from bootstrap samples
  ...
  #  boot_CI_lower=boot_res.quantile(alpha/2)  # get lower CI
   ...
   # std error based off bootstrap samples:
    ...
    names = ['boot_res','alpha_boot','omega_boot','eta_boot','scale1_boot','scale2_boot',\
    ...
    out=(boot_res,alpha_boot_clean,omega_boot_clean,eta_boot_clean,scale1_boot_clean,scale2_boot_clean,\
         alpha_CI_lower,alpha_CI_upper,\
         omega_CI_lower,omega_CI_upper,\
         eta_CI_lower,eta_CI_upper,\
         scale1_CI_lower,scale1_CI_upper,\
