...
def result_csv(single_sinusoids,boot_samples,k,int_area,omega,SF,O1,O1p,save_output=False):
    # amplitudes ratio
    k=len(omega)
    ...
    j_mat=calc_couplings(omega=omega, k=k) # frequency distance matrix 
    
    idx_list=getoverlaps(boot_samples,omega_ppm, k) # comment out if no boostrap samples have been genreated
    ...
    res_uncertantinty=pd.DataFrame(({'omega_hz_error':boot_samples['omega_std'],
                                'omega_CI_low': boot_samples['omega_CI_lower']+O1+(omega-np.median(boot_samples['omega_boot'],axis=1)),
                                'omega_CI_up': boot_samples['omega_CI_upper']+O1+(omega-np.median(boot_samples['omega_boot'],axis=1)),
                                'omega_ppm_error':hz2ppm(omega_hz=boot_samples['omega_boot'], SF=SF, O1p=O1p).T.std(),
                                'omega_ppm_CI_lower':hz2ppm(boot_samples['omega_CI_upper']+O1+(omega-np.median(boot_samples['omega_boot'],axis=1)), SF=SF, O1p=O1p),
                                'omega_ppm_CI_upper':hz2ppm(boot_samples['omega_CI_upper']+O1+(omega-np.median(boot_samples['omega_boot'],axis=1)), SF=SF, O1p=O1p)
                                }))
    ...
    res_region=pd.DataFrame(({'amp_ratio':amp_ratios,
                             ...
                             'amp_height': raw_amps,
                             'integral' : int_area,
                             'int_ratio': int_area/np.max(int_area)
                             }))
...
def result_csv_noboot(single_sinusoids,k,int_area,omega,SF,O1,O1p,save_output=False):
    # amplitudes ratio
    k=len(omega)
    amp_ratios=amp_ratio(peaks=single_sinusoids, k=k)
    raw_amps=get_amps(peaks=single_sinusoids, k=k)
