...
# -*- coding: utf-8 -*-
"""
Created on Sat Nov 19 18:57:36 2022
...
def onion_noskew_genvoigt(omega,tn,t,fs,y):
    N=len(y)
   ...
   # init_scale2=np.repeat(0.0,k)        
    ...
    init2=np.asarray((init_a,init_f,init_e,init_scale))
    ...
    class sinusoid_refit(torch.nn.Module):
            ...
            def forward(self, t_in,y_in,k):
                # signal pole matrix
                 ...
                 # weigthed decay
                 decay=torch.exp(torch.pow(torch.outer(t_in,ones.repeat(k)),SP(self.e))*-SP(self.scale)*expit(self.a))              
                # model matrix Z(theta)
                 Z=Z_poles*decay
                 # small diagnonal to aviod instability
    ...
    optimizer=torch.optim.LBFGS(model.parameters(), lr=0.1, max_iter=15, tolerance_grad=1e-5, tolerance_change=1e-09, history_size=100, line_search_fn='strong_wolfe')
   ...
   # scale2=(model_out[4]).detach().numpy()
    ...
    parms_k=np.concatenate((alpha1,omega1,eta1,Softplus(scale1)))
